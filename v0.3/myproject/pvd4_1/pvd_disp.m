...
flag1=1;%1-启用数据处理，0-取消数据处理
flag2=1;%1-启用st，0-取消st
flag3=1;%1-DRLR,0-LR
weight=7;%flag3-flag2-flag1:0-7间的数
%%%读取数据
data=readcell('sample_test.xlsx','Sheet','PS3');
...
if flag2==1
    ...
    if flag1==0
        ...
        if flag3==0
            ...
            data_b=readcell('weight.xlsx','Sheet','b');%DRLR+处理
        end
    end
    b=cell2mat(data_b);
...
data_theta=readcell('weight.xlsx','Sheet','w');%w为训练集无错误情况下求得的权重，仅在111（全开）情况下可以用，用来跑最佳结果
